...
=======
          ...
          if(editMode && _activeInp && _activeInp.value.startsWith('=')){
            ...
            var ref = cellId(rr,cc);
            var cur = _activeInp.value;
            // Only replace a trailing partial cell reference (letters+digits at end, not after open paren or operator)
            var newVal = cur.replace(/([,(+\-*/^(])[A-Z]{0,2}\d*$/, function(m, op){ return op + ref; });
            // If nothing was replaced (ref at very start after =), just append
            ...
            // Visually highlight the selected cell without triggering selectCell side effects
...
=======
        td.addEventListener('mouseup',function(e){
          ...
          if(editMode && _activeInp && _activeInp.value.startsWith('=') && rangeStart){
            var r1=Math.min(rangeStart.r,rangeEnd?rangeEnd.r:rr);
            var r2=Math.max(rangeStart.r,rangeEnd?rangeEnd.r:rr);
            var c1=Math.min(rangeStart.c,rangeEnd?rangeEnd.c:cc);
            var c2=Math.max(rangeStart.c,rangeEnd?rangeEnd.c:cc);
            if(r1!==r2||c1!==c2){
        ...
        td.addEventListener('mouseover',function(e){
          if(editMode && _activeInp && _activeInp.value.startsWith('=') && e.buttons===1 && rangeStart){
            rangeEnd={r:rr,c:cc};
            // Show live range highlight
            document.querySelectorAll('.dc.formula-sel').forEach(function(el){el.classList.remove('formula-sel');});
            var r1=Math.min(rangeStart.r,rr),r2=Math.max(rangeStart.r,rr);
            var c1=Math.min(rangeStart.c,cc),c2=Math.max(rangeStart.c,cc);
            for(var fr=r1;fr<=r2;fr++)for(var fc=c1;fc<=c2;fc++){
              var fel=document.getElementById('c-'+fr+'-'+fc);
              if(fel)fel.classList.add('formula-sel');
            }
            return;
          }
          extSel(rr,cc,e);
...
function extSel(r,c,e){
  ...
  // If editing a formula, update the cell reference in the input
  if(editMode && _activeInp && _activeInp.value.startsWith('=')){
    ...
    var cur=_activeInp.value;
    _activeInp.value=cur.replace(/[A-Z]+\d+(:[A-Z]+\d+)?$/,'')+ref;
    document.getElementById('formula-input').value=_activeInp.value;
  }
}
...
document.addEventListener('keydown',function(e){
  ...
  if(nav[e.key]){var d=nav[e.key];selectCell(Math.max(0,Math.min(ROWS-1,selR+d[0])),Math.max(0,Math.min(COLS-1,selC+d[1])));e.preventDefault();return;}
  ...
  if(e.key==='Home'){selectCell(selR,0);e.preventDefault();return;}
  if(e.key==='End'){selectCell(selR,COLS-1);e.preventDefault();return;}
...
function openFontColor(){var c=prompt('Enter color (e.g. #ff0000 or red):','#000000');if(c)applyStyleProp('color',c);}
function openFillColor(){var c=prompt('Enter fill color (e.g. #ffff00 or lightyellow):','#ffffff');if(c)applyStyleProp('bg',c);}
